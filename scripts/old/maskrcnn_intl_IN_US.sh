#!/bin/bash
#SBATCH --job-name=maskrcnn_IN_US
...
#SBATCH --output=logs/maskrcnn_IN_US_%j.out
#SBATCH --error=logs/maskrcnn_IN_US_%j.err

set -euo pipefail

# -----------------------------------------------------------------------------
# Mask R-CNN: International shift — train India (South), test USA (North). 1 GPU.
# ~18k train images, batch 12, 30 epochs -> 45k iterations.
# -----------------------------------------------------------------------------
...
OUTPUT_DIR=./outputs/maskrcnn_IN_US_${SLURM_JOB_ID}
