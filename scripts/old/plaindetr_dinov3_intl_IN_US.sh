#!/bin/bash
#SBATCH --job-name=plaindetr_dinov3_IN_US
...
#SBATCH --output=logs/plaindetr_dinov3_IN_US_%j.out
#SBATCH --error=logs/plaindetr_dinov3_IN_US_%j.err

# -----------------------------------------------------------------------------
# Plain-DETR with DinoV3 backbone: train India, test USA.
# Uses tree-shift Apptainer container. Training + eval + univariate + Shapley.
#
# Uses dinov3_vits16 (~22M params) - fits on 40GB GPUs. Uses dinov3_vits.pth.
# -----------------------------------------------------------------------------
...
OUTPUT_DIR="${REPO_ROOT}/outputs/plaindetr_dinov3_IN_US_${SLURM_JOB_ID}"
...
echo "=== Plain-DETR DinoV3: train India, OOD USA ==="
...
# Use torchrun for multi-GPU; avoids scontrol (not in container) required by Plain-DETR's Slurm path
# NCCL_IB_DISABLE=1 and NCCL_SOCKET_IFNAME fix IPv6 errno 97 on Sherlock
NUM_GPUS=2
