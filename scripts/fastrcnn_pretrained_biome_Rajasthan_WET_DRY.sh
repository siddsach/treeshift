#!/bin/bash
#SBATCH --job-name=frcnn_pt_biome_Rajasthan_WET_DRY
...
#SBATCH --output=logs/fastrcnn_pretrained_biome_Rajasthan_WET_DRY_%j.out
#SBATCH --error=logs/fastrcnn_pretrained_biome_Rajasthan_WET_DRY_%j.err

# -----------------------------------------------------------------------
# Faster R-CNN (COCO pretrained): biome_Rajasthan_train_WET__ood_DRY
# 1714 train images, batch 8 (1 GPU), 50 epochs = 10750 iterations.
...
OUTPUT_DIR="${REPO_ROOT}/outputs/fastrcnn_pretrained_biome_Rajasthan_WET_DRY_${SLURM_JOB_ID}"
