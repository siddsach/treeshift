#!/bin/bash
#SBATCH --job-name=frcnn_pt_biome_Rajasthan_DRY_WET
...
#SBATCH --output=logs/fastrcnn_pretrained_biome_Rajasthan_DRY_WET_%j.out
#SBATCH --error=logs/fastrcnn_pretrained_biome_Rajasthan_DRY_WET_%j.err

# -----------------------------------------------------------------------
# Faster R-CNN (COCO pretrained): biome_Rajasthan_train_DRY__ood_WET
# 1934 train images, batch 8 (1 GPU), 50 epochs = 12100 iterations.
...
OUTPUT_DIR="${REPO_ROOT}/outputs/fastrcnn_pretrained_biome_Rajasthan_DRY_WET_${SLURM_JOB_ID}"
