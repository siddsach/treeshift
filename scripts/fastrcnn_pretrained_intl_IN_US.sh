#!/bin/bash
#SBATCH --job-name=frcnn_pt_intl_IN_US
...
#SBATCH --output=logs/fastrcnn_pretrained_intl_IN_US_%j.out
#SBATCH --error=logs/fastrcnn_pretrained_intl_IN_US_%j.err

# -----------------------------------------------------------------------
# Faster R-CNN (COCO pretrained): intl_train_IN__ood_US
# 12143 train images, batch 24 (2 GPUs), 50 epochs = 25300 iterations.
...
OUTPUT_DIR="${REPO_ROOT}/outputs/fastrcnn_pretrained_intl_IN_US_${SLURM_JOB_ID}"
