#!/bin/bash
#SBATCH --job-name=frcnn_pt_intl_US_IN
...
#SBATCH --output=logs/fastrcnn_pretrained_intl_US_IN_%j.out
#SBATCH --error=logs/fastrcnn_pretrained_intl_US_IN_%j.err

# -----------------------------------------------------------------------
# Faster R-CNN (COCO pretrained): intl_train_US__ood_IN
# 8923 train images, batch 10 (1 GPU), 40 epochs = 35720 iterations.
...
OUTPUT_DIR="${REPO_ROOT}/outputs/fastrcnn_pretrained_intl_US_IN_1gpu40ep_${SLURM_JOB_ID}"
