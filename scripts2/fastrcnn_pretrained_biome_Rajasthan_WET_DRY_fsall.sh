#!/bin/bash
#SBATCH --job-name=frcnn_pt_biome_Rajasthan_WET_DRY_fsall
...
#SBATCH --output=logs/fastrcnn_pretrained_biome_Rajasthan_WET_DRY_fsall_%j.out
#SBATCH --error=logs/fastrcnn_pretrained_biome_Rajasthan_WET_DRY_fsall_%j.err

# -----------------------------------------------------------------------
# Faster R-CNN (COCO pretrained): biome_Rajasthan_train_WET__ood_DRY__fsall
# 3218 train images, batch 8 (1 GPU), 40 epochs = 16120 iterations.
...
OUTPUT_DIR="${REPO_ROOT}/outputs/fastrcnn_pretrained_biome_Rajasthan_WET_DRY_fsall_1gpu40ep_${SLURM_JOB_ID}"
