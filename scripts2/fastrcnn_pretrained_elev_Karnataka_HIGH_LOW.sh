#!/bin/bash
#SBATCH --job-name=frcnn_pt_elev_Karnataka_HIGH_LOW
...
#SBATCH --output=logs/fastrcnn_pretrained_elev_Karnataka_HIGH_LOW_%j.out
#SBATCH --error=logs/fastrcnn_pretrained_elev_Karnataka_HIGH_LOW_%j.err

# -----------------------------------------------------------------------
# Faster R-CNN (COCO pretrained): elev_Karnataka_train_HIGH__ood_LOW
# 2351 train images, batch 8 (1 GPU), 40 epochs = 11760 iterations.
...
OUTPUT_DIR="${REPO_ROOT}/outputs/fastrcnn_pretrained_elev_Karnataka_HIGH_LOW_1gpu40ep_${SLURM_JOB_ID}"
