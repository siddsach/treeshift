#!/bin/bash
#SBATCH --job-name=gdino_intl_US_IN_fs1
...
#SBATCH --output=logs/grounding_dino_intl_US_IN_fs1_%j.out
#SBATCH --error=logs/grounding_dino_intl_US_IN_fs1_%j.err

# -----------------------------------------------------------------------
# MM-Grounding-DINO: intl_train_US__ood_IN__fs1
# 8924 train images, batch 2/GPU × 1 GPU, 40 epochs max.
...
OUTPUT_DIR="${REPO_ROOT}/outputs/grounding_dino_intl_US_IN_fs1_1gpu40ep_${SLURM_JOB_ID}"
NUM_GPUS=1
...
echo "=== GroundingDINO: intl_train_US__ood_IN__fs1 ==="
