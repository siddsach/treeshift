#!/usr/bin/env python3
"""
Detectron2 Mask R-CNN Training and Inference Script
Data is loaded via the ``tree_shift`` pip package (>= 0.2.0).

Usage (recommended — tree_shift auto-download):
    # Train only
    python detectron_maskrcnn.py run --config intl_train_IN__ood_US --train

    # Train + evaluate val + evaluate OOD
    python detectron_maskrcnn.py run --config intl_train_IN__ood_US --train --eval-val --eval-ood

    # Evaluate only (requires trained model)
    python detectron_maskrcnn.py run --config intl_train_IN__ood_US --eval-val --eval-ood --model-path ./output/model_final.pth

    # With explicit COCO export directory
    python detectron_maskrcnn.py run --config intl_train_IN__ood_US --coco-dir ./my_coco --train --eval-val --eval-ood

Usage (legacy — manual paths):
    python detectron_maskrcnn.py train --train-annotations train/annotations.json --train-images train/images
    python detectron_maskrcnn.py predict --model-path ./output/model_final.pth --input-image test.jpg --output-image output.jpg
    python detectron_maskrcnn.py evaluate --model-path ./output/model_final.pth --test-annotations test/annotations.json --test-images test/images
"""
...
def _distributed_train(cfg, resume=True, train_annotations=None, train_images=None):
    """Helper function for distributed training."""
    if train_annotations and train_images:
        if os.path.exists(train_annotations) and os.path.exists(train_images):
            ...
            if dataset_name not in DatasetCatalog.list():
                ...
                print(f"Re-registered training dataset in GPU process: {train_annotations}")
            else:
                print(f"Dataset '{dataset_name}' already registered in GPU process. Skipping re-registration.")
...
def _filter_segmentation_annotations(annotation_file):
    """Filter out annotations without valid segmentation masks from a COCO JSON.

    Mask R-CNN requires every annotation to have a non-empty 'segmentation'
    field.  A small number of annotations may be missing masks.  This function
    loads the JSON, drops those annotations, writes the cleaned version back,
    and returns the (unchanged) file path.
    """
    ...
    data["annotations"] = [
        ...
        if a.get("segmentation") and a["segmentation"] != [] and a["segmentation"] != [[]]
    ]
    ...
    if dropped > 0:
        with open(annotation_file, "w") as f:
            json.dump(data, f)
        print(f"Filtered {dropped} annotations without segmentation masks from {annotation_file} ({before} -> {after})")
...
def register_tree_shift_datasets(paths, register_train=True, register_val=True,
    ...
    for name, ann, imgs in splits:
        ...
        _filter_segmentation_annotations(ann)
...
def register_datasets(train_annotations=None, train_images=None, test_annotations=None, test_images=None):
    ...
    if os.path.exists(train_annotations) and os.path.exists(train_images):
        _filter_segmentation_annotations(train_annotations)
    ...
    if os.path.exists(test_annotations) and os.path.exists(test_images):
        _filter_segmentation_annotations(test_annotations)
...
def create_trainer_config(num_classes=1, batch_size=4, learning_rate=0.0002,
    ...
    if config_file and os.path.exists(config_file):
        ...
        # Use Mask R-CNN for instance segmentation (bounding box + masks)
        _config_yaml = os.path.join(os.path.dirname(os.path.abspath(__file__)), "detectron2", "configs", "COCO-InstanceSegmentation", "mask_rcnn_R_50_FPN_3x.yaml")
        cfg.merge_from_file(_config_yaml)
        if pretrained:
            cfg.MODEL.WEIGHTS = model_zoo.get_checkpoint_url("COCO-InstanceSegmentation/mask_rcnn_R_50_FPN_3x.yaml")
            print("Using Mask R-CNN config with COCO pretrained weights")
        else:
            cfg.MODEL.WEIGHTS = ""  # train from scratch
            print("Using Mask R-CNN config (train from scratch)")
    ...
    # Use multiple workers for faster data loading (16 workers for 64-core HPC nodes)
    # With 64 cores available, we can maximize parallel data preprocessing
    cfg.DATALOADER.NUM_WORKERS = 8
    
    # CRITICAL: Set mask format to 'bitmask' to handle RLE masks
...
def generate_eval_summary_table(all_results, config_name, output_path, model_type="Mask R-CNN"):
    """Generate a table image summarising AP metrics across eval splits.

    Columns (Mask R-CNN includes segm):
        Train - Test | AP@0.50 [bbox] | AP@[0.50:0.95] [bbox] | AP@0.50 [segm] | APs [bbox] | APm [bbox] | APl [bbox]
    ...
    # Create figure
    n_rows = len(rows)
    fig_width = max(16, len(columns) * 1.5)
...
def evaluate_model(predictor, dataset_name="test", output_dir="./detectron_output"):
    ...
    original_dicts = DatasetCatalog.get(dataset_name)
    print(f"Original dataset: {len(original_dicts)} images")
    
    # Filter for annotations with segmentation masks (COCO eval needs them)
    filtered_dicts, total_anns, kept_anns = filter_annotations_with_segmentation(original_dicts)
    skipped_images = len(original_dicts) - len(filtered_dicts)
    
    print(f"Filtered dataset: {len(filtered_dicts)} images ({skipped_images} skipped without segmentation)")
    print(f"Annotations: {kept_anns}/{total_anns} have segmentation masks")
    
    coco_results = {}
    if len(filtered_dicts) > 0:
        import uuid
        filtered_dataset_name = f"{dataset_name}_filtered_{str(uuid.uuid4())[:8]}"
        DatasetCatalog.register(filtered_dataset_name, lambda: filtered_dicts)
        
        if dataset_name in MetadataCatalog.list():
            original_metadata = MetadataCatalog.get(dataset_name)
            filtered_metadata = MetadataCatalog.get(filtered_dataset_name)
            if hasattr(original_metadata, 'thing_classes'):
                filtered_metadata.set(thing_classes=original_metadata.thing_classes)
        
        evaluator = COCOEvaluator(filtered_dataset_name, output_dir=output_dir)
        val_loader = build_detection_test_loader(predictor.cfg, filtered_dataset_name)
        
        print(f"Evaluating Mask R-CNN on {len(filtered_dicts)} images with segmentation masks...")
        coco_results = inference_on_dataset(predictor.model, val_loader, evaluator)
        
        print("COCO Evaluation results:")
        print(json.dumps(coco_results, indent=2))
    else:
        print("WARNING: No images with segmentation masks found for COCO eval.")

    coco_bbox_stats = coco_results.get("bbox", {})

    # Collect per-image predictions on ALL images for unified evaluation
    image_records = []
    ...
    if coco_results.get("segm"):
        results["segm"] = coco_results["segm"]
    return results


def filter_annotations_with_segmentation(dataset_dicts):
    """Filter dataset to only include annotations with valid segmentation masks."""
    filtered_dicts = []
    total_anns = 0
    kept_anns = 0
    ...
    for d in dataset_dicts:
        ...
        for ann in d.get('annotations', []):
            total_anns += 1
            # Keep annotation only if it has segmentation
            if 'segmentation' in ann and ann['segmentation']:
                # Additional validation: check if segmentation is not empty
                segm = ann['segmentation']
                if isinstance(segm, dict):  # RLE format
                    if 'counts' in segm and segm['counts']:
        ...
        if filtered_anns:
            d_copy = d.copy()
            d_copy['annotations'] = filtered_anns
            filtered_dicts.append(d_copy)
    
    return filtered_dicts, total_anns, kept_anns
...
def evaluate_model_by_zone(predictor, dataset_name="test", output_dir="./detectron_output",
    ...
    for zone, zone_dicts in zone_datasets.items():
        ...
        # Filter out images without segmentation masks for Mask R-CNN
        filtered_zone_dicts, total_anns, kept_anns = filter_annotations_with_segmentation(zone_dicts)
        skipped_images = len(zone_dicts) - len(filtered_zone_dicts)
        
        print(f"  Original: {len(zone_dicts)} images, {total_anns} annotations")
        print(f"  Filtered: {len(filtered_zone_dicts)} images, {kept_anns} annotations with segmentation masks")
        print(f"  Skipped: {skipped_images} images without valid segmentation masks")
        
        if len(filtered_zone_dicts) == 0:
            print(f"  WARNING: No images with segmentation masks in zone {zone}. Skipping evaluation.")
            zone_results[zone] = {
                'error': 'No images with segmentation masks',
        ...
        # Register zone-specific dataset with filtered data
        DatasetCatalog.register(zone_dataset_name, lambda filtered_zone_dicts=filtered_zone_dicts: filtered_zone_dicts)
        ...
        try:
            ...
            zone_results[zone] = {
                'metrics': results,
                'num_images': len(zone_dicts),
                'num_images_evaluated': len(filtered_zone_dicts),
                'num_annotations_total': total_anns,
                'num_annotations_with_segmentation': kept_anns,
                'output_dir': zone_output_dir
            }
            print(f"Zone {zone} results: {results.get('segm', {})}")
...
def learning_rate_range_test(train_annotations, train_images, 
                             min_lr=1e-5, max_lr=1e-2, 
                             iterations=2000, batch_size=4,
                             output_dir="./lr_range_test"):
    """
    Learning Rate Range Test - finds optimal learning rate in one training run.
    
    This method trains a model while exponentially increasing the learning rate
    from very low to very high, logging the loss at each step. The optimal LR
    is typically found where the loss is decreasing fastest (steepest negative slope).
    
    Args:
        train_annotations: Path to training annotations JSON
        train_images: Path to training images directory
        min_lr: Starting learning rate (default: 1e-5, very low)
        max_lr: Ending learning rate (default: 1e-2, fairly high for Mask R-CNN)
        iterations: Total iterations for the test (default: 2000, ~1-2 hours)
        batch_size: Batch size for training (default: 4)
        output_dir: Where to save results and plots
    
    Returns:
        dict: Results containing LR history, loss history, and optimal LR
    """
    if not DETECTRON2_AVAILABLE:
        print("Error: detectron2 is not installed. Please install with: pip install -r requirements.txt")
        return None
    ...
    print(f"Estimated time: ~1-2 hours on GPU, 4-8 hours on CPU")
    print(f"Output directory: {output_dir}")
    print("="*70)
    
    # Register dataset
    register_datasets(train_annotations=train_annotations, train_images=train_images)
    ...
    # Fixed parameters for LR test
    cfg.SOLVER.IMS_PER_BATCH = batch_size
    cfg.SOLVER.BASE_LR = min_lr  # Start with min_lr
    cfg.SOLVER.MAX_ITER = iterations
    cfg.SOLVER.STEPS = []  # No decay during LR test
    cfg.SOLVER.WARMUP_ITERS = 0  # No warmup for LR test
    cfg.MODEL.ROI_HEADS.BATCH_SIZE_PER_IMAGE = 512
    ...
    # Custom trainer that increases LR and logs data
    class LRRangeTestTrainer(DefaultTrainer):
        def __init__(self, cfg, min_lr, max_lr, iterations):
            super().__init__(cfg)
            self.min_lr = min_lr
            self.max_lr = max_lr
        ...
        def run_step(self):
            """Override run_step to update LR exponentially each iteration"""
            # Calculate current LR (exponential increase)
            current_iter = self.iter
            progress = current_iter / self.total_iterations
            # Exponential interpolation: lr = min_lr * (max_lr/min_lr)^progress
            ...
            if current_iter % 100 == 0:
                print(f"Iter {current_iter}/{self.total_iterations}: LR={current_lr:.6f}, Loss={total_loss:.4f}")
    
    # Run training
    print("\nStarting LR Range Test training...")
    trainer = LRRangeTestTrainer(cfg, min_lr, max_lr, iterations)
    ...
    results_file = os.path.join(output_dir, "lr_range_test_results.json")
    with open(results_file, 'w') as f:
    ...
    if MATPLOTLIB_AVAILABLE and len(trainer.lr_history) > 50:
        print("\nAnalyzing results...")
        
        # Smooth the loss curve to find optimal LR
        lr_array = np.array(trainer.lr_history)
        loss_array = np.array(trainer.loss_history)
        
        # Remove any NaN or inf values
        valid_mask = np.isfinite(loss_array)
        lr_array = lr_array[valid_mask]
        loss_array = loss_array[valid_mask]
        
        if len(lr_array) > 50:
            # Find optimal LR (steepest descent before divergence)
            # Use gradient to find where loss is decreasing fastest
            try:
                # Smooth with moving average
                window_size = min(50, len(loss_array) // 10)
                if window_size > 1:
                    loss_smooth = np.convolve(loss_array, np.ones(window_size)/window_size, mode='valid')
                    lr_smooth = lr_array[window_size//2:len(loss_smooth)+window_size//2]
                else:
                    loss_smooth = loss_array
                    lr_smooth = lr_array
                
                # Calculate negative gradient (rate of decrease)
                loss_gradient = np.gradient(loss_smooth)
                
                # Find where gradient is most negative (steepest descent)
                # But exclude the very beginning (first 10% of iterations)
                start_idx = len(loss_gradient) // 10
                min_gradient_idx = start_idx + np.argmin(loss_gradient[start_idx:])
                optimal_lr = lr_smooth[min_gradient_idx]
                
                # Also find where loss stops decreasing (divergence point)
                # Look for where gradient becomes positive
                divergence_idx = np.where(loss_gradient > 0)[0]
                if len(divergence_idx) > 0:
                    divergence_lr = lr_smooth[divergence_idx[0]]
                else:
                    divergence_lr = max_lr
                
                print(f"\n{'='*70}")
                print("ANALYSIS RESULTS")
                print(f"{'='*70}")
                print(f"Optimal Learning Rate (steepest descent): {optimal_lr:.6f}")
                print(f"Safe upper bound (before divergence): {divergence_lr:.6f}")
                print(f"\nRecommended LR range for grid search:")
                print(f"  Conservative: {optimal_lr/3:.6f} to {optimal_lr:.6f}")
                print(f"  Moderate:     {optimal_lr/2:.6f} to {optimal_lr*2:.6f}")
                print(f"  Aggressive:   {optimal_lr:.6f} to {min(optimal_lr*3, divergence_lr):.6f}")
                print(f"{'='*70}")
                
                results['optimal_lr'] = float(optimal_lr)
                results['divergence_lr'] = float(divergence_lr)
                results['recommended_range'] = {
                    'conservative': [float(optimal_lr/3), float(optimal_lr)],
                    'moderate': [float(optimal_lr/2), float(optimal_lr*2)],
                    'aggressive': [float(optimal_lr), float(min(optimal_lr*3, divergence_lr))]
                }
                
            except Exception as e:
                print(f"Warning: Could not calculate optimal LR: {e}")
                optimal_lr = None
            ...
            ax1.plot(lr_array, loss_array, 'b-', linewidth=1, alpha=0.6, label='Raw loss')
            if len(loss_smooth) > 0:
                ax1.plot(lr_smooth, loss_smooth, 'r-', linewidth=2, label='Smoothed loss')
            ax1.set_xscale('log')
            ax1.set_xlabel('Learning Rate (log scale)', fontsize=12)
            ax1.set_ylabel('Loss', fontsize=12)
            ax1.set_title('Learning Rate Range Test - Loss vs Learning Rate', fontsize=14, fontweight='bold')
            ax1.grid(True, alpha=0.3)
            
            if optimal_lr:
                ax1.axvline(x=optimal_lr, color='green', linestyle='--', linewidth=2,
                           label=f'Optimal LR: {optimal_lr:.6f}')
                if divergence_lr < max_lr:
                    ax1.axvline(x=divergence_lr, color='red', linestyle='--', linewidth=2,
                               label=f'Divergence: {divergence_lr:.6f}')
            
            ax1.legend(fontsize=10)
            
            # Plot 2: Loss vs Iteration
            ax2.plot(range(len(trainer.loss_history)), trainer.loss_history, 'b-', linewidth=1)
            ax2.set_xlabel('Iteration', fontsize=12)
            ax2.set_ylabel('Loss', fontsize=12)
            ax2.set_title('Loss vs Iteration (LR increasing exponentially)', fontsize=14, fontweight='bold')
            ax2.grid(True, alpha=0.3)
            
            plt.tight_layout()
            plot_file = os.path.join(output_dir, "lr_range_test.png")
        ...
        print("Matplotlib not available or insufficient data - skipping plot generation")
...
def main():
    parser = argparse.ArgumentParser(description="Detectron2 Mask R-CNN Training and Inference Script")
    ...
    # ---- Legacy subcommands ----
    # Learning Rate Range Test command
    lr_test_parser = subparsers.add_parser('lr_test', help='Run Learning Rate Range Test to find optimal LR')
    lr_test_parser.add_argument('--train-annotations', required=True, help='Path to training annotations JSON')
    lr_test_parser.add_argument('--train-images', required=True, help='Path to training images directory')
    lr_test_parser.add_argument('--min-lr', type=float, default=1e-5, help='Minimum learning rate (default: 1e-5)')
    lr_test_parser.add_argument('--max-lr', type=float, default=1e-2, help='Maximum learning rate (default: 1e-2)')
    lr_test_parser.add_argument('--iterations', type=int, default=2000, help='Number of iterations (default: 2000)')
    lr_test_parser.add_argument('--batch-size', type=int, default=4, help='Batch size (default: 4)')
    lr_test_parser.add_argument('--output-dir', default='./lr_range_test', help='Output directory')
    ...
    # Train and Evaluate command (runs both sequentially)
    train_eval_parser = subparsers.add_parser('train_and_evaluate', help='Train model then evaluate on test set')
    train_eval_parser.add_argument('--train-annotations', default=None, help='Path to training annotations JSON')
    train_eval_parser.add_argument('--train-images', default=None, help='Path to training images directory')
    train_eval_parser.add_argument('--test-annotations', required=True, help='Path to test annotations JSON')
    train_eval_parser.add_argument('--test-images', required=True, help='Path to test images directory')
    train_eval_parser.add_argument('--num-classes', type=int, default=1, help='Number of classes (1 for Kejri Tree only)')
    train_eval_parser.add_argument('--batch-size', type=int, default=4, help='Batch size')
    train_eval_parser.add_argument('--learning-rate', type=float, default=0.0002, help='Learning rate')
    train_eval_parser.add_argument('--max-iterations', type=int, default=90000, help='Maximum iterations')
    train_eval_parser.add_argument('--output-dir', default='./output', help='Training output directory')
    train_eval_parser.add_argument('--eval-output-dir', default='./evaluation_output', help='Evaluation output directory')
    train_eval_parser.add_argument('--config-file', default=None, help='Custom config file path')
    train_eval_parser.add_argument('--score-threshold', type=float, default=0.7, help='Confidence threshold for evaluation')
    train_eval_parser.add_argument('--resume', action='store_true', default=True, help='Resume from checkpoint')
    train_eval_parser.add_argument('--visualize', action='store_true', help='Visualize training data')
    train_eval_parser.add_argument('--visualize-comparison', action='store_true', help='Visualize predictions vs ground truth')
    ...
    if args.command == 'run':
        ...
        if eval_splits:
            ...
            for split_name, res in all_results.items():
                ...
                if segm:
                    print(f"    segm  — AP={segm.get('AP', 'N/A'):.3f}  AP50={segm.get('AP50', 'N/A'):.3f}  AP75={segm.get('AP75', 'N/A'):.3f}")
                tm = res.get("tree_metrics", {}).get("overall", {})
                if tm:
            ...
            if all_results:
                ...
                generate_eval_summary_table(table_results, args.config, table_path, model_type="Mask R-CNN")
            else:
                print("ERROR: No splits evaluated successfully — no summary written.")
                sys.exit(1)

    elif args.command == 'lr_test':
        print("=== Learning Rate Range Test Mode ===")
        
        # Run LR range test
        results = learning_rate_range_test(
        ...
        if results:
            print("\n✅ Learning Rate Range Test completed successfully!")
            if 'optimal_lr' in results:
                print(f"\n🎯 Optimal Learning Rate: {results['optimal_lr']:.6f}")
                print("\nNext steps:")
                print("1. Review the plot: {}/lr_range_test.png".format(args.output_dir))
                print("2. Use the recommended LR range for grid search or training")
                print("3. Example: python detectron_maskrcnn.py train --learning-rate {:.6f}".format(results['optimal_lr']))
        else:
            print("❌ Learning Rate Range Test failed!")
        ...
        # Force single GPU training for better performance with data workers
        # Multi-GPU training with 0 workers is slower due to data loading bottleneck
        print("Using single GPU training with multiple data workers for optimal performance")
        trainer = train_model(cfg, resume=args.resume)
        ...
        print("=== Train and Evaluate Mode ===")
        print("This will train the model first, then evaluate on test set")
        print()
        ...
        if not predictor:
            print("Failed to create predictor! Aborting evaluation.")
            sys.exit(1)
        
        # Run evaluation
        results = evaluate_model(predictor, "test", args.eval_output_dir)
        ...
        print("SUMMARY")
        print("=" * 60)
        print(f"Training completed: {args.output_dir}/model_final.pth")
        print(f"Evaluation results: {args.eval_output_dir}/")
        
        if results:
            print("✅ Train and Evaluate completed successfully!")
        else:
            print("⚠️  Training succeeded but evaluation had issues")
