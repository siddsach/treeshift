...
def get_args_parser():
    ...
    parser.add_argument("--early_stop_min_epochs", default=30, type=int)
    ...
    # tree_shift COCO export layout (tree-distribution-shift project)
    parser.add_argument("--hf_coco_dir", default=None, type=str,
                        help="Root of tree_shift COCO export dir. When set with --hf_coco_config, "
                             "overrides --coco_path with the project's split layout.")
    parser.add_argument("--hf_coco_config", default=None, type=str,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.early_stop and ("coco_eval_bbox" in test_stats):
            ...
            if stop:
                if utils.is_main_process():
                    print(
                        f"Early stopping triggered at epoch {epoch}. Best AP={early_stop_best:.4f} at epoch {early_stop_best_epoch}"
                    )
                break
