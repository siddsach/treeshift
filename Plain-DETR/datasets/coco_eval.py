...
class CocoEvaluator(object):
    ...
    def prepare_for_coco_detection(self, predictions):
        # Model outputs 0-indexed labels; COCO uses dataset category_ids (e.g. 1 for tree).
        # Map label i -> coco_gt.catIds[i] so predictions match GT.
        ...
        for original_id, prediction in predictions.items():
            ...
            for k, box in enumerate(boxes):
                lab = labels[k]
                if lab >= len(cat_ids):  # no-object class or invalid
                    continue
                coco_results.append({
